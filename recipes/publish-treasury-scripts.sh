...
Arguments:
  ...
  SCOPE     Which scope to publish scripts for (core_development, ops_and_use_cases, network_compliance, middleware or contingency)
...
echo -e "\n\033[1mBuild transaction publishing $SCOPE treasury scripts?\033[0m (requires a running cardano-node)" >&2
ask_to_continue
...
OUT=build/publish-treasury-scripts-$SCOPE.tx.json

CLI_OUT=$(cardano-cli conway transaction build \
   $NETWORK_FLAG \
   --tx-in "$FUEL_TX#$FUEL_IX" \
   $(treasury-script $SCOPE $CHANGE_ADDR) \
